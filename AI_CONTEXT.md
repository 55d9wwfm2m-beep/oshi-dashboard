...
## 2026-09-12 UIリニューアル（以下の旧UI記述より優先）

- 今回の明示依頼により、§13-10のホーム外観維持・§11のセリフ数字／ライト固定を更新。**計算・保存・既存機能を維持したUI全面変更**。
- `site/yarikuri.css` はPages版とNext.js版で共有。Next側は `app/money/layout.tsx` で読み込み、`.money-app` でやりくりだけに限定する。推し活の外観は変更しない。
- 両版ともダーク固定。背景 `#0b0d0c`、surface `#131715`、elevated `#1c221f`、主文字 `#f2f5f3`、補助文字 `#b4bdb8`、mint `#88cfc2`。金額はOSのsystem sans-serif、tabular-nums。Apple専用フォント・アセットは同梱しない。
- `components/ui/money-theme.ts` は既存の状態ラベルを引き継いで表示色のみ指定する。**`lib/money.ts`、`types/index.ts`、`hooks/useLocalStorage.ts` は未変更**。
- 両版でホーム／残高／予算／目標／振り返り／設定の6タブを維持し、線画アイコン付き下部Materialナビに統一。Nextの推し活ナビは `/money` のときのみ非表示。推し活のページ・ナビは存続。
- 「今月のお金」の4本の配分バーは既存内訳の表示専用。割合のクランプを金額・計算へ戻さない。残高は中間の架空額へカウントせず、実額更新に短いfadeを添える。
- 保存されているカテゴリーの `emoji` フィールド・編集機能は維持し、主要UIの絵文字アイコンを線画へ変更。
- localStorageキー・データ形式は不変。**同名キーでも別originのブラウザ間でデータが共有されるわけではない**。公開先を変更しないことが既存データ継続の前提。
- PWAに不足していたアイコンを独自のwallet線画から生成。SWには共有CSSを追加、キャッシュ名を `yarikuri-ui-v2` へ更新。他アプリのキャッシュは削除しない。
- `prefers-reduced-motion`、キーボードフォーカス、44px操作領域、safe-areaに対応。半透明はナビゲーションに限定。
- 参考: https://developer.apple.com/design/human-interface-guidelines/ および https://developer.apple.com/videos/play/wwdc2025/219/
- 検証: production build・TypeScriptチェック成功。旧版／新版の同一シードによるDOM回帰テスト74項目を通過（残高・固定費の支払／確定額・口座切替／編集・給料／実貯金・支出登録・目標編集／99.9%表示・再読込・旧単一残高移行・月替わり）。テストデータは独立したメモリ内DOMのみで使用し、本番には保存していない。
- **公開・更新の方針（ユーザー指定）**: やりくりの変更は原則、実装・検証・既存URLへの公開反映まで行う。確認用ブランチへの保存だけで止めない。`claude/mobile-money-calculator-ndye1f` の `site/**` 更新で既存のGitHub ActionsがPagesへ公開する。既存origin・start_url・localStorageを維持し、ホーム画面に追加したアプリも従来のネットワーク優先キャッシュから更新を受け取れる状態を保つ。
- **検証上の制限**: 今回は実ブラウザによる画面・コンソール・iPhone操作の最終確認が、作業環境でブラウザ起動が制限されたため未完了。ビルド・型チェック・DOM回帰テストとは区別する。この制限を報告した後、ユーザーから公開反映まで進めるよう明示指示を受けた。未実施の画面確認を成功扱いしない。

以下はUI変更前の仕様の記録。計算・保存仕様は引き続き有効だが、配色・書体・ナビ位置・行番号は上記と実装を優先する。

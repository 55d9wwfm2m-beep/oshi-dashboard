...
import { OshiProfile, OshiEvent, AttendanceLog, Expense, Savings, AvatarConfig, DEFAULT_AVATAR_CONFIG } from '@/types';
import { ACHIEVEMENTS, RARITY_STYLE, checkAchievement, getLevelTier } from '@/lib/game';
import Avatar, { AvatarItemGrid, NextUnlock } from '@/components/Avatar';
...
export default function AchievementsPage() {
  ...
  const earned = ACHIEVEMENTS.filter(a => checkAchievement(a.id, checkData));
  const locked = ACHIEVEMENTS.filter(a => !checkAchievement(a.id, checkData));
    ...
    <div className="min-h-screen">
      ...
      <div className="px-4 space-y-4 pb-4">
        ...
        <div className="card p-5 anim-scaleIn">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-2">
                <NextUnlock level={gameState.level} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card p-5 anim-fadeInUp stagger-1">
          <p className="text-sm font-medium mb-4" style={{ color: '#78716C' }}>アバターアイテム</p>
          <AvatarItemGrid level={gameState.level} oshiColor={avatarConfig.oshiColor} />
        </div>

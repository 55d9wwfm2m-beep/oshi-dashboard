...
export default function RoadmapPage() {
  ...
  const paceStyle = info && (info.pace === 'done' || info.pace === 'ahead')
    ...
    : info?.pace === 'close'
      ? { background: 'rgba(168,119,14,0.10)', color: WARN }
      : { background: 'rgba(196,87,75,0.10)', color: MONEY_DANGER };

  return (
    <div className="min-h-screen">
      <div className="px-4 pt-8 pb-4 flex items-end justify-between gap-3">
        <div className="anim-fadeIn min-w-0">
          <p className="text-[11px] font-medium tracking-widest uppercase" style={{ color: '#A8A29E' }}>Roadmap</p>
          <h1 className="text-xl font-semibold mt-0.5" style={{ color: '#1C1917', letterSpacing: '-0.02em' }}>
            30歳までのロードマップ
          </h1>
        </div>
        <button
      ...
      <div className="px-4 space-y-4">
        {/* 次の目標 */}
        <div
          className="card p-5 text-center anim-fadeInUp"
          style={{ background: `linear-gradient(168deg, ${MONEY_ACCENT_BG}, #fff 62%)` }}
        >
          {!info ? (
            <>
              <p className="text-[11.5px] font-bold tracking-widest" style={{ color: '#78716C' }}>次の目標</p>
              <p className="text-xs mt-2.5 leading-relaxed" style={{ color: '#78716C' }}>
            ...
            <>
              ...
              <p className="text-xs mt-1" style={{ color: '#A8A29E' }}>{info.goal.name}</p>
              ...
              >
                <div
                  ...
                  style={{ width: `${pct}%`, background: info.pace === 'done' ? '#2B7A63' : MONEY_ACCENT }}
                />
              </div>
              <div className="flex justify-between text-[11.5px]" style={{ color: '#A8A29E', fontVariantNumeric: 'tabular-nums' }}>
              ...
              <div className="grid grid-cols-2 gap-2.5 mt-4">
                {[{ k: '現在', v: info.current }, { k: 'あと', v: info.remaining }].map(cell => (
                  <div key={cell.k} className="rounded-2xl px-2.5 py-3" style={{ background: '#FAF8F6' }}>
                    <p className="text-[11px]" style={{ color: '#78716C' }}>{cell.k}</p>
                    <p className="text-[18px] font-bold font-serif-num mt-0.5" style={{ color: '#1C1917' }}>
                      {formatYen(cell.v)}
                    </p>
                  </div>
                ))}
              ...
              <p className="text-xs mt-2" style={{ color: '#78716C' }}>{pace!.note}</p>

              {info.remaining > 0 && (
                <p
                  className="mt-3.5 px-3.5 py-2.5 rounded-2xl text-[12.5px] font-semibold"
                  style={{ background: '#F0EBE6', color: '#1C1917' }}
                >
                  目標達成には、ここから月平均{' '}
        ...
        <div className="card p-5 anim-fadeInUp">
          <div className="flex items-center justify-between mb-3.5">
            <p className="text-sm font-medium" style={{ color: '#78716C' }}>目標の一覧</p>
          ...
          {list.length === 0 ? (
            <p className="text-xs text-center py-2.5" style={{ color: '#A8A29E' }}>目標がまだありません</p>
          ) : (
            list.map((m, i) => {
                ...
                <div key={m.id} className="flex gap-3">
                  <div className="w-[22px] shrink-0 flex flex-col items-center">
                    <div
                      ...
                      style={{
                        border: `2px solid ${isEvent ? WARN : done || isCurrent ? MONEY_ACCENT : 'rgba(28,18,12,0.12)'}`,
                        background: done ? MONEY_ACCENT : isEvent ? 'rgba(168,119,14,0.10)' : '#fff',
                    ...
                    {i < list.length - 1 && (
                      <div className="flex-1 w-0.5 my-[3px] min-h-[14px]" style={{ background: 'rgba(28,18,12,0.10)' }} />
                    )}
                  </div>
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-[11.5px] font-semibold" style={{ color: isCurrent ? MONEY_ACCENT : '#A8A29E' }}>
                          {formatMonthLabel(m.month)}{isCurrent && '・いま向かっている目標'}
                        </p>
                        <p
                          className="text-[17px] font-bold mt-px"
                          style={{
                            color: isEvent ? WARN : done ? '#78716C' : isCurrent ? '#1C1917' : '#A8A29E',
                            fontVariantNumeric: 'tabular-nums',
                          }}
                        >
                          {isEvent ? `🚙 −${formatYen(m.amount)}` : formatMan(m.amount)}
                        </p>
                        <p className="text-[11.5px] mt-0.5" style={{ color: '#A8A29E' }}>
                          {m.name}{done && '・達成'}
                        </p>
                      </div>
                      <button
                    ...
                    {isEvent && typeof m.after === 'number' && (
                      <p
                        ...
                        style={{ color: '#78716C', borderTop: '1px dashed rgba(28,18,12,0.12)' }}
                      >
        ...
        <p className="text-[11px] text-center pb-2" style={{ color: '#A8A29E' }}>
          目標はいつでも追加・変更できます
        </p>
      </div>
          ...
          <>
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-1.5 p-1 rounded-2xl" style={{ background: '#F0EBE6' }} role="group" aria-label="目標の種類">
      ...
      <BottomSheet open={sourceOpen} onClose={() => setSourceOpen(false)} title="ロードマップ対象残高">
        <p className="text-[11px] mb-3" style={{ color: '#A8A29E' }}>どのお金を貯金として数えるかを決めます</p>

        <div className="flex gap-1.5 p-1 rounded-2xl mb-4" style={{ background: '#F0EBE6' }} role="group" aria-label="対象残高の決め方">
        ...
        {roadmap.source === 'accounts' ? (
          <div>
            ...
            {accounts.length === 0 ? (
              <p className="text-xs text-center py-2.5" style={{ color: '#A8A29E' }}>口座が登録されていません</p>
            ) : (
              accounts.map(a => (

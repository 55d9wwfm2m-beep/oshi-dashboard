...
export function Legs({ skin, pants, ids }) {
  ...
  const shoe = (cx) => (
    <g key={`sh${cx}`}>
      <ellipse cx={cx} cy="333" rx="10" ry="3.2" fill="#CBBEDF" filter={ids.soft1} />
      ...
      <ellipse cx={cx - 2} cy="322" rx="3" ry="2" fill="#FFF" opacity=".5" filter={ids.soft1} />
    </g>
  );
  const leg = (cx) => (
    <g key={`lg${cx}`}>
      <path d={`M${cx},248 C${cx - 1},278 ${cx - 1},300 ${cx},312`} stroke={legColor} strokeWidth={pants ? 13 : 10.5} fill="none" strokeLinecap="round" />
      <path d={`M${cx - 3},258 C${cx - 4},280 ${cx - 4},296 ${cx - 3},308`} stroke={legShade} strokeWidth="3" fill="none" strokeLinecap="round" opacity=".35" filter={ids.soft1} />
      <path d={`M${cx + 3},262 C${cx + 3},280 ${cx + 3},296 ${cx + 3},306`} stroke={legHi} strokeWidth="2" fill="none" strokeLinecap="round" opacity={pants ? ".3" : ".4"} filter={ids.soft1} />
    </g>
  );
...
export function Arms({ skin, ids }) {
  ...
  const arm = (sx, hx, sign) => (
    <g key={`a${sign}`}>
      ...
      <path d={`M${sx + sign * 2},202 C${sx + sign * 7},212 ${hx + sign * 3},228 ${hx + sign * 1},240`} stroke={s.shade} strokeWidth="2.4" fill="none" strokeLinecap="round" opacity=".3" filter={ids.soft1} />
    </g>
  );
